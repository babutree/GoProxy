    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>★</th><th>名称 / 备注</th><th>协议</th><th>地域</th><th>出口 IP<span class="muted"> (信息)</span></th><th>延迟</th><th>ipapi.is 滥用分<span class="muted"> /1.00</span></th><th>ip-api 标记</th><th>CF 拦截</th><th>来源</th><th>状态</th><th>操作</th></tr></thead>
            <tbody id="proxy-rows"><tr><td colspan="12" class="empty">加载中</td></tr></tbody>
          </table>
        </div>
      </div>
    </div>
...
// copyProxyCred: 复制完整代理 URL 协议://用户名DSL:密码@IP:端口。用户名 DSL 沿用 <base>-region-<region>-session-<rand6>;
// 密码取 configCache.proxy_auth_password(为空则留空并提示未配置);dual_protocol 节点 confirm 选 socks5/http,单协议节点用 p.protocol。
function copyProxyCred(id){ const p=allProxies.find(x=>Number(x.id)===Number(id)); if(!p)return; const base=(configCache&&configCache.proxy_auth_username)?configCache.proxy_auth_username:'acct'; const region=regionOf(p); const user=base+'-region-'+region+'-session-'+randSession(); const pass=(configCache&&configCache.proxy_auth_password)?configCache.proxy_auth_password:''; const addr=String(p.address||''); const scheme=isDualProtocol(p)?(confirm('确定复制 SOCKS5？取消则复制 HTTP')?'socks5':'http'):String(p.protocol||'socks5'); const url=scheme+'://'+user+':'+pass+'@'+addr; if(!pass)showToast('代理密码未配置'); navigator.clipboard.writeText(url).then(()=>showToast('已复制: '+url)).catch(()=>showToast('复制失败')) }
...
function renderProxies(){const protocol=document.getElementById('protocol-filter').value;const region=document.getElementById('region-filter').value;const sf=document.getElementById('status-filter').value;let rows=allProxies.filter(p=>(!protocol||p.protocol===protocol)&&(!region||regionOf(p)===region));if(sf)rows=rows.filter(p=>nodeState(p)===sf);const order={ok:0,pending:1,paused:2,failed:3};rows.sort((a,b)=>{const fa=(nodeState(a)==='ok'&&(a.starred===true||Number(a.starred)===1))?1:0;const fb=(nodeState(b)==='ok'&&(b.starred===true||Number(b.starred)===1))?1:0;if(fa!==fb)return fb-fa;const sa=nodeState(a),sb=nodeState(b);if(order[sa]!==order[sb])return order[sa]-order[sb];return Number(a.latency||1e9)-Number(b.latency||1e9)});const body=document.getElementById('proxy-rows');if(rows.length===0){body.innerHTML='<tr><td colspan="12" class="empty">没有匹配节点</td></tr>';return}body.innerHTML=rows.map(p=>{const addr=addressArg(p.address);const id=proxyIDArg(p);const manual=p.source==='manual';const st=nodeState(p);const label=html(nodeLabel(p));const showRegion=isAvailable(p)&&isKnownRegion(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),false)">停用</button>';const testBtn='<button class="mini" onclick="testProxy('+id+',decodeURIComponent(\''+addr+'\'))">测试</button>';const copyBtn='<button class="mini" onclick="copyProxyCred('+id+')">复制</button>';const actions=manual?('<button class="mini" onclick="editManualRegion('+id+',decodeURIComponent(\''+addr+'\'))">地域</button> <button class="mini" onclick="editManualNote('+id+',decodeURIComponent(\''+addr+'\'))">备注</button> '+testBtn+' '+copyBtn+' '+toggleBtn+' <button class="mini danger" onclick="deleteManualNode('+id+',decodeURIComponent(\''+addr+'\'))">删除</button>'):(testBtn+' '+copyBtn+' '+toggleBtn);const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';return '<tr><td>'+starBtn(p)+'</td><td title="'+label+'">'+label+'</td><td>'+protocolBadges(p)+'</td><td>'+(showRegion?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+abuserBadge(p.ipapiis_score)+'</td><td>'+ipapiFlagsBadges(p.ipapi_flags,!!p.ipapi_flags_seen)+'</td><td>'+cfBadge(p.cf_blocked)+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td>'+actions+'</td></tr>'}).join('')}
