        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="table-wrap">
              <table>
                <thead><tr><th>协议</th><th>地址</th><th>地域</th><th>出口 IP</th><th>延迟</th><th>来源</th><th>状态</th><th>操作</th></tr></thead>
...
function renderProxies(){const protocol=document.getElementById('protocol-filter').value;const region=document.getElementById('region-filter').value;const rows=allProxies.filter(p=>(!protocol||p.protocol===protocol)&&(!region||regionOf(p)===region));const body=document.getElementById('proxy-rows');if(rows.length===0){body.innerHTML='<tr><td colspan="8" class="empty">没有匹配节点</td></tr>';return}body.innerHTML=rows.map(p=>{const masked=html(maskAddress(p.address));const addr=addressArg(p.address);const source=safe(p.source);const actions=source==='manual'?'<button class="mini" onclick="editManualRegion(decodeURIComponent(\''+addr+'\'))">地域</button> <button class="mini" onclick="editManualNote(decodeURIComponent(\''+addr+'\'))">备注</button> <button class="mini danger" onclick="deleteManualNode(decodeURIComponent(\''+addr+'\'))">删除</button>':'<span class="muted">订阅只读</span>';return '<tr><td><span class="badge blue">'+html(p.protocol).toUpperCase()+'</span></td><td class="mono" title="'+masked+'">'+masked+'</td><td><span class="badge green">'+html(regionOf(p)).toUpperCase()+'</span></td><td class="mono">'+html(p.exit_ip)+'</td><td>'+html(p.latency)+' ms</td><td>'+html(source)+'</td><td><span class="badge gray" title="'+html(p.note)+'">'+html(p.status)+'</span></td><td>'+actions+'</td></tr>'}).join('')}
...
refreshAll().catch(err=>showToast(err.message));setInterval(loadLogs,5000);setInterval(loadSessions,15000);
</script>
</body>
</html>`
