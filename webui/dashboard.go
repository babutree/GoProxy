  ...
  <section class="page active" id="page-overview">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="cmd" id="conn-cmd">curl --socks5 acct:PASSWORD@127.0.0.1:7801 https://httpbin.org/ip</div>
...
async function refreshAll(){await Promise.all([loadStats(),loadProxies(),loadSubscriptions(),loadConfig(),loadSessions(),loadLogs(),loadCustomStatus()]);loadPublicIP();showToast('数据已刷新')}
async function loadCustomStatus(){const st=await api('/api/custom/status');if(!st)return;const box=document.getElementById('singbox-status');if(!box)return;const running=st.singbox_running;const dot='<span class="dot '+(running?'on':'off')+'"></span>';box.innerHTML='<div class="kv"><span>'+dot+'sing-box 引擎</span><span class="v">'+(running?'运行中':'未运行')+'</span></div>'+'<div class="kv"><span class="k">转换节点</span><span class="v">'+safe(st.singbox_nodes)+'</span></div>'+'<div class="kv"><span class="k">订阅可用</span><span class="v">'+safe(st.subscription_count)+'</span></div>'+'<div class="kv"><span class="k">禁用节点</span><span class="v">'+safe(st.disabled_count)+'</span></div>'+'<div class="kv"><span class="k">订阅总数</span><span class="v">'+safe(st.subscription_total)+'</span></div>'}
...
function isVerified(p){return Number(p.latency)>0&&!!p.region&&p.region!=='unknown'}
// 节点状态归类: ok(可用) / paused(手动停用) / failed(验证失败) / pending(待验证)
function nodeState(p){if(p.status==='paused')return 'paused';if(p.status==='disabled')return 'failed';if(isVerified(p))return 'ok';return 'pending'}
...
function renderProxies(){const protocol=document.getElementById('protocol-filter').value;const region=document.getElementById('region-filter').value;const sf=document.getElementById('status-filter').value;let rows=allProxies.filter(p=>(!protocol||p.protocol===protocol)&&(!region||regionOf(p)===region));if(sf)rows=rows.filter(p=>nodeState(p)===sf);const order={ok:0,pending:1,paused:2,failed:3};rows.sort((a,b)=>{const sa=nodeState(a),sb=nodeState(b);if(order[sa]!==order[sb])return order[sa]-order[sb];return Number(a.latency||1e9)-Number(b.latency||1e9)});const body=document.getElementById('proxy-rows');if(rows.length===0){body.innerHTML='<tr><td colspan="8" class="empty">没有匹配节点</td></tr>';return}body.innerHTML=rows.map(p=>{const addr=addressArg(p.address);const manual=p.source==='manual';const st=nodeState(p);const label=html(nodeLabel(p));const verified=isVerified(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy(decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy(decodeURIComponent(\''+addr+'\'),false)">停用</button>';const actions=manual?('<button class="mini" onclick="editManualRegion(decodeURIComponent(\''+addr+'\'))">地域</button> <button class="mini" onclick="editManualNote(decodeURIComponent(\''+addr+'\'))">备注</button> '+toggleBtn+' <button class="mini danger" onclick="deleteManualNode(decodeURIComponent(\''+addr+'\'))">删除</button>'):toggleBtn;const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';return '<tr><td title="'+label+'">'+label+'</td><td><span class="badge blue">'+html(p.protocol).toUpperCase()+'</span></td><td>'+(verified?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td>'+actions+'</td></tr>'}).join('')}
...
async function loadSubscriptions(){const subs=await api('/api/subscriptions');if(!subs)return;const box=document.getElementById('sub-list');if(!Array.isArray(subs)||subs.length===0){box.innerHTML='<div class="empty">暂无订阅，点右上角“添加订阅”</div>';return}box.innerHTML=subs.map(sub=>{const paused=sub.status==='paused';const toggleLabel=paused?'启用':'暂停';const badge=paused?'<span class="badge warn">已暂停</span>':'<span class="badge ok">活跃</span>';return '<div class="sub-item"><div class="meta"><strong>'+html(sub.name)+' '+badge+'</strong><div class="muted">'+safe(sub.active_count)+' 可用 / '+safe(sub.disabled_count)+' 禁用</div></div><div class="mini-actions"><button class="mini" onclick="refreshSub('+sub.id+')" title="重新拉取并验证">刷新</button><button class="mini" onclick="toggleSub('+sub.id+')" title="启用或暂停该订阅及其节点">'+toggleLabel+'</button><button class="mini danger" onclick="deleteSub('+sub.id+')" title="删除订阅及其节点">删除</button></div></div>'}).join('')}
...
async function loadPublicIP(){try{const d=await api('/api/public-ip');if(d&&d.public_ip){publicIP=d.public_ip;renderConnection()}}catch(e){}}
function renderConnection(){if(!configCache)return;const sp=stripColon(configCache.socks5_port)||'7801';const hp=stripColon(configCache.http_port)||'7802';const base=configCache.proxy_auth_username||'acct';const enabled=configCache.proxy_auth_enabled;const host=publicIP||location.hostname||'127.0.0.1';document.getElementById('conn-socks5').textContent=host+':'+(sp||'7801');document.getElementById('conn-http').textContent=host+':'+(hp||'7802');document.getElementById('conn-user').textContent=base;document.getElementById('conn-pass').textContent=enabled?'见首次启动日志 / 系统设置':'（认证已关闭，无需密码）';document.getElementById('conn-auth-state').textContent=enabled?'代理认证：开启':'代理认证：关闭';const cred=enabled?(base+':PASSWORD@'):'';document.getElementById('conn-cmd').textContent='curl --socks5 '+cred+host+':'+(sp||'7801')+' https://httpbin.org/ip'}
