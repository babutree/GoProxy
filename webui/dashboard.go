    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>名称 / 备注</th><th>协议</th><th>地域</th><th>出口 IP<span class="muted"> (信息)</span></th><th>延迟</th><th>ipapi.is 滥用分<span class="muted"> /1.00</span></th><th>ip-api 标记</th><th>来源</th><th>状态</th><th>操作</th></tr></thead>
            <tbody id="proxy-rows"><tr><td colspan="10" class="empty">加载中</td></tr></tbody>
          </table>
        </div>
      </div>
    </div>
...
// ipapiFlagsBadges: ip-api 命中标记逗号串。proxy 红、hosting 黄、mobile 灰；已探测(exit_ip 非空)但无命中显"干净"绿；未探测显 "--"。
function ipapiFlagsBadges(flags,probed){const raw=String(flags||'').trim();if(raw===''){return probed?'<span class="badge ok">干净</span>':'<span class="muted">--</span>'}const cls={proxy:'danger',hosting:'warn',mobile:'gray'};return raw.split(',').map(f=>f.trim()).filter(Boolean).map(f=>'<span class="badge '+(cls[f]||'gray')+'">'+html(f)+'</span>').join(' ')}
function renderProxies(){const protocol=document.getElementById('protocol-filter').value;const region=document.getElementById('region-filter').value;const sf=document.getElementById('status-filter').value;let rows=allProxies.filter(p=>(!protocol||p.protocol===protocol)&&(!region||regionOf(p)===region));if(sf)rows=rows.filter(p=>nodeState(p)===sf);const order={ok:0,pending:1,paused:2,failed:3};rows.sort((a,b)=>{const sa=nodeState(a),sb=nodeState(b);if(order[sa]!==order[sb])return order[sa]-order[sb];return Number(a.latency||1e9)-Number(b.latency||1e9)});const body=document.getElementById('proxy-rows');if(rows.length===0){body.innerHTML='<tr><td colspan="10" class="empty">没有匹配节点</td></tr>';return}body.innerHTML=rows.map(p=>{const addr=addressArg(p.address);const id=proxyIDArg(p);const manual=p.source==='manual';const st=nodeState(p);const label=html(nodeLabel(p));const showRegion=isAvailable(p)&&isKnownRegion(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),false)">停用</button>';const testBtn='<button class="mini" onclick="testProxy('+id+',decodeURIComponent(\''+addr+'\'))">测试</button>';const actions=manual?('<button class="mini" onclick="editManualRegion('+id+',decodeURIComponent(\''+addr+'\'))">地域</button> <button class="mini" onclick="editManualNote('+id+',decodeURIComponent(\''+addr+'\'))">备注</button> '+testBtn+' '+toggleBtn+' <button class="mini danger" onclick="deleteManualNode('+id+',decodeURIComponent(\''+addr+'\'))">删除</button>'):(testBtn+' '+toggleBtn);const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';return '<tr><td title="'+label+'">'+label+'</td><td><span class="badge blue">'+html(p.protocol).toUpperCase()+'</span></td><td>'+(showRegion?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+abuserBadge(p.ipapiis_score)+'</td><td>'+ipapiFlagsBadges(p.ipapi_flags,!!p.exit_ip)+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td>'+actions+'</td></tr>'}).join('')}
