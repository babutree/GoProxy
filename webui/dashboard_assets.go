...
.region-head{display:grid;grid-template-columns:52px minmax(88px,0.7fr) minmax(0,1.4fr) auto;gap:10px 14px;align-items:center;padding:0 14px 6px;color:var(--muted);font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase}
.region{display:grid;grid-template-columns:52px minmax(88px,0.7fr) minmax(0,1.4fr) auto;gap:10px 14px;align-items:center;padding:12px 14px;border:1px solid var(--hairline);border-radius:12px;background:var(--panel-2)}
.region .cc{font-weight:800;font-size:15px;letter-spacing:.04em;line-height:1.1;font-family:"Consolas",monospace}
.region .name{font-weight:700;font-size:13px;color:var(--ink);line-height:1.25;min-width:0;word-break:break-word}
...
function nodeLabel(p){if(p.note)return p.note;return maskAddress(p.address);}
...
function regionRowHTML(item,maxN){const pct=maxN?Math.round(item.count*100/maxN):0;const avg=item.avgLat>0?(item.avgLat+' ms'):'--';const chips=[];if(item.s)chips.push('<span class="badge qs">S '+html(item.s)+'</span>');if(item.a)chips.push('<span class="badge qa">A '+html(item.a)+'</span>');if(item.b)chips.push('<span class="badge qb">B '+html(item.b)+'</span>');if(item.c)chips.push('<span class="badge qc">C '+html(item.c)+'</span>');if(item.d)chips.push('<span class="badge qd">D '+html(item.d)+'</span>');if(item.sess)chips.push('<span class="badge ok">会话 '+html(item.sess)+'</span>');chips.push('<span class="badge gray">均延 '+html(avg)+'</span>');return '<div class="region"><span class="cc">'+html(item.region).toUpperCase()+'</span><span class="name">'+html(regionDisplayName(item.region))+'</span><div class="meta"><div class="bar"><i style="width:'+pct+'%"></i></div><div class="chips">'+chips.join('')+'</div></div><div class="n"><span class="big num">'+html(item.count)+'</span><span class="sub">节点</span></div></div>'}
function regionPanelHTML(entries,maxN,limit){if(!entries.length)return '<div class="empty">暂无可用地域数据</div>';const rows=typeof limit==='number'?entries.slice(0,limit):entries;return '<div class="region-head"><span>代码</span><span>国家/地区</span><span>结构 / 延迟 / 会话</span><span>节点</span></div>'+rows.map(function(item){return regionRowHTML(item,maxN)}).join('')}
...
function proxyRowHTML(p){const addr=addressArg(p.address);const id=proxyIDArg(p);const manual=p.source==='manual';const st=nodeState(p);const label=html(nodeLabel(p));const showRegion=isAvailable(p)&&isKnownRegion(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),false)">停用</button>';const testBtn='<button class="mini" onclick="testProxy('+id+',decodeURIComponent(\''+addr+'\'))">测试</button>';const copyBtn='<button class="mini" onclick="copyProxyCred('+id+')">复制</button>';const baseActions=testBtn+' '+copyBtn+' '+toggleBtn;const manageBtn='<button class="mini" onclick="manageNode('+id+')">管理</button>';const actions=baseActions+' '+manageBtn;const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';const sel='<input type="checkbox" class="proxy-select" value="'+id+'">';const nameCell='<td class="note-edit" title="'+label+' · 点击编辑备注" style="cursor:pointer" onclick="editNote('+id+')">'+label+'</td>';return '<tr><td>'+sel+'</td><td>'+starBtn(p)+'</td>'+nameCell+'<td>'+protocolBadges(p)+'</td><td>'+(showRegion?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+abuserBadge(p.ipapiis_score)+'</td><td>'+ipapiFlagsBadges(p.ipapi_flags,!!p.ipapi_flags_seen)+'</td><td>'+cfBadge(p.cf_blocked)+'</td><td>'+aiBadges(p.ai_reachability)+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td><span class="ops">'+actions+'</span></td></tr>'}
