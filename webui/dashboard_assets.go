...
async function loadLogs(){const data=await api('/api/logs');if(!data)return;const box=document.getElementById('logs-box');if(!box)return;const auto=document.getElementById('logs-autoscroll');const follow=!!(auto&&auto.checked);const prevTop=box.scrollTop;const oldElements=follow?[]:Array.from(box.children);const oldLines=oldElements.map(el=>el.textContent||'');let anchorIndex=-1,anchorOffset=0;if(!follow){const boxTop=box.getBoundingClientRect().top;anchorIndex=oldElements.findIndex(el=>el.getBoundingClientRect().bottom>boxTop);if(anchorIndex>=0)anchorOffset=oldElements[anchorIndex].getBoundingClientRect().top-boxTop}const lines=Array.isArray(data.lines)?data.lines:[];box.innerHTML=lines.length?lines.map(line=>'<div class="log-line">'+html(line)+'</div>').join(''):'<div class="log-line">no logs</div>';if(follow){box.scrollTop=box.scrollHeight;return}const shift=logWindowShift(oldLines,lines);const mappedIndex=shift===null?-1:anchorIndex-shift;const fresh=Array.from(box.children);if(mappedIndex>=0&&mappedIndex<fresh.length){const boxTop=box.getBoundingClientRect().top;const currentOffset=fresh[mappedIndex].getBoundingClientRect().top-boxTop;box.scrollTop=Math.max(0,box.scrollTop+currentOffset-anchorOffset);return}box.scrollTop=prevTop}
...
function proxyRowHTML(p){const addr=addressArg(p.address);const id=proxyIDArg(p);const manual=p.source==='manual';const st=nodeState(p);const labelText=nodeLabel(p);const label=labelText?html(labelText):'';const showRegion=isAvailable(p)&&isKnownRegion(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),false)">停用</button>';const testBtn='<button class="mini" onclick="testProxy('+id+',decodeURIComponent(\''+addr+'\'))">测试</button>';const copyBtn='<button class="mini" onclick="copyProxyCred('+id+')">复制</button>';const baseActions=testBtn+' '+copyBtn+' '+toggleBtn;const manageBtn='<button class="mini" onclick="manageNode('+id+')">管理</button>';const actions=baseActions+' '+manageBtn;const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';const sel='<input type="checkbox" class="proxy-select" value="'+id+'">';const nameTitle=label?(label+' · 点击编辑备注'):'点击添加备注';const nameCell='<td class="note-edit" title="'+nameTitle+'" style="cursor:pointer" onclick="editNote('+id+')">'+label+'</td>';return '<tr><td>'+sel+'</td><td>'+starBtn(p)+'</td>'+nameCell+'<td>'+protocolBadges(p)+'</td><td>'+(showRegion?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+abuserBadge(p.ipapiis_score)+'</td><td>'+ipapiFlagsBadges(p.ipapi_flags,!!p.ipapi_flags_seen)+'</td><td>'+cfBadge(p.cf_blocked)+'</td><td>'+aiBadges(p.ai_reachability)+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td><span class="ops">'+actions+'</span></td></tr>'}
